...
class BookCommentController extends Controller {
     ...
     */
     public function agree(Request $request){
         ...
         $commentExists = DB::table('book_comment_opinion')
            ->where('comment_id', $id)
        ...
        if($commentExists) {    // If entry update
            ...
            $valid = DB::table('book_comment_opinion')
                ->where('comment_id', $id)
                ->where('email', Auth::user()->email)
    ...
    public function approve(Request $request){
        ...
        if( Auth::user()->role_id == 7) {
               ...
               $valid = $update = DB::table('book_comment')->where('comment_id', $id)->update(
                [
                    'comment_status' => 1
                ]
            );
    ...
    */
    public function delete(Request $request)
    {
        ...
        if(Auth::user()->role_id == 7 || DB::table('book_comment')->where('comment_id', $id)->where('email', Auth::user()->email)->exists())
        {
            $valid = DB::table('book_comment')->where('comment_id', $id)->delete();          
        }
    ...
    /*public function updateComment(Request $request)
    {
        ...
        $rating = $request->input('editcomment_rating');
        $valid = false;
        
        $valid = DB::table('book_comment')->where('book_id', $id)->where('email', Auth::user()->email)->update(
        [
            'text' => $text,
            'rating' => $rating
        ]
    ...
    */
       public function report(Request $request){
        ...
        $comment = DB::table('book_comment_opinion')
            ->where('comment_id', $id)
            ->where('email', $email)
            ->first();
    ...
    */
    public function insertComment(Request $request){
        ...
        $rating = $request->input('comment_rating');
        ...
        $valid = DB::table('book_comment')->insert(
            [
                'book_id' => $id,
                'email' => Auth::user()->email,
                'alias' => $name,
    ...
    */
    public function updateComment(Request $request)
    {
        ...
        if(Auth::user()->role_id == 7)
        {
            $valid = DB::table('book_comment')->where('comment_id', $commentID)->update(
            [
                'text' => $text,
                'rating' => $rating
            ]
